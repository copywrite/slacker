...
<sqlMap namespace="${daoClassName}">
  ...
  <!--
           ...
           是否有缓存：无
    -->
  <insert id="${daoClassName}.insert" parameterClass="${doClassName}">
    <![CDATA[
    INSERT INTO `${tableName}`(
    ]]>
    <include refid="${tableName}-allCols"/>
    <![CDATA[
    )VALUES(
  <#list columnList as column>
    <#if column_index==0>         <#else>
      ,</#if><#if column.name=="GMT_MODIFIED" || column.name=="GMT_CREATE"|| column.name=="GMT_CREATED">
    now()<#elseif column.name=="IS_DELETED" ||  column.name=="DELETED">0<#else>#${column.property}#</#if>
  </#list>
    );
    ]]>
  <#if daoUseSequence>
  ...
  <!--
           ...
           是否有缓存：无
    -->
  <update id="${daoClassName}.update">
    <![CDATA[
    UPDATE `${tableName}` SET <#if haveGmtModified>`GMT_MODIFIED` = now()<#else>`${primaryKeyColumn.name}` =
    #${primaryKeyColumn.property}#;</#if>
    ]]>
    <dynamic>
    <#list columnList as column>
  ...
  <!--
           ...
           是否有缓存：无
        -->
  <select id="${daoClassName}.select" resultMap="result${doClassName}Map">
    <![CDATA[
    SELECT
    ]]>
    <include refid="${tableName}-allCols"/>
  ...
  <!--
           ...
           1.
    -->
  <select id="${daoClassName}.queryList" resultMap="result${doClassName}Map">
    <![CDATA[
    SELECT
    ]]>
    <include refid="${tableName}-allCols"/>
  ...
  <!--
           ...
           1.
    -->
  <select id="${daoClassName}.queryCount" resultClass="java.lang.Integer">
    <![CDATA[
    SELECT
    COUNT(*)
